...
Usage:
  python3 scms.py start   [server|agent|both]
  python3 scms.py stop    [server|agent|both]
  python3 scms.py restart [server|agent|both]
  python3 scms.py status
  python3 scms.py logs    [server|agent]  [-n LINES]
"""
...
def cmd_start(name: str):
    ...
    logfile = open(svc["logfile"], "a")   # noqa: WPS515
...
def cmd_stop(name: str, timeout: int = 10):
    svc     = SERVICES[name]
    running, pid = _is_running(name)
    if not running:
...
def cmd_logs(name: str, lines: int):
    svc = SERVICES[name]
